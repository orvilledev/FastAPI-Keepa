...
export type FbaBoxContentsResult = {
  blob: Blob
  filename: string
  rowCount: number
  boxCount: number
...
export const fbaBoxContentsApi = {
  generate: async (file: File): Promise<FbaBoxContentsResult> => {
    ...
    form.append('file', file)
    try {
      const response = await api.post<Blob>('/api/v1/fba-box-contents/generate', form, {
        responseType: 'blob',
        timeout: 120_000,
      })
      const headers = response.headers || {}
...
export const catalogShipToApi = {
  list: async (
    ...
    offset = 0,
    search?: string
  ): Promise<CatalogShipToListResponse> => {
    const response = await api.get('/api/v1/catalog-ship-to', {
      params: { limit, offset, search: search || undefined },
    })
    return response.data
  },
