...
export default function Sidebar() {
  ...
  const toolsMenuItems = [
    { path: '/howtoguide', label: 'How To Guide', icon: 'globe' as const },
    ...
    <aside className="w-64 bg-white/80 backdrop-blur-lg border-r border-gray-200/80 shadow-lg h-screen sticky top-0 z-50">
      ...
      >
        <div className="space-y-1">
          ...
          <Link
            to="/assistant"
            onMouseEnter={() => setHoveredNav('assistant')}
            className={`sidebar-link ${
              navHighlighted('assistant', isActive('/assistant'))
                ? 'sidebar-link-active'
                : 'sidebar-link-inactive'
            }`}
          >
            <span className="mr-3">{Icons.chat}</span>
            <span>Assistant</span>
          </Link>
