...
export default function DNKDailyRun() {
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setError('')
      await schedulerApi.updateSettings(settingsForm, 'dnk')
  ...
  const handleInputModeChange = async (mode: 'api' | 'uploaded') => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ input_mode: mode }, 'dnk')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Daily Runs List */}
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">DNK Daily Run History</h2>
        {dailyRuns.length === 0 ? (
          <div className="card p-12 text-center">
            <div className="text-gray-500 mb-4">No DNK daily runs found yet.</div>
            <p className="text-sm text-gray-400">
              DNK daily runs are automatically executed at {formatScheduledTime()}.
            </p>
          </div>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Schedule timing and email recipients here apply to DNK only.
                </p>
              </div>
            </div>
