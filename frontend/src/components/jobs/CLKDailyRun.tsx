...
export default function CLKDailyRun() {
  ...
  const handleToggleEnabled = async () => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ enabled: newEnabled }, 'clk')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
        ...
        <div className="card p-4 bg-yellow-50 border-yellow-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">The CLK daily scheduler is currently disabled. Click "Start Daily Run" to resume.</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Schedule timing and email recipients here apply to CLK only.
                </p>
              </div>
            </div>
