...
export default function BORDailyRun() {
  ...
  const loadNextRun = async () => {
    try {
      const data = await schedulerApi.getNextRun('bor')
      setNextRun(data)
    } catch (err: any) {
  ...
  const loadSchedulerSettings = async () => {
    try {
      const settings = await schedulerApi.getSettings('bor')
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setError('')
      await schedulerApi.updateSettings(settingsForm, 'bor')
  ...
  const handleToggleEnabled = async () => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ enabled: newEnabled }, 'bor')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
  ...
  const handleInputModeChange = async (mode: 'api' | 'uploaded') => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ input_mode: mode }, 'bor')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
  ...
  const loadDailyRuns = async () => {
    try {
      ...
      const dailyJobs = allJobs.filter((job: any) =>
        job.job_name &&
        (job.job_name.startsWith('Daily BOR Off Price Report -') ||
          job.job_name.startsWith('Daily BOR Metro Report -')),
      )
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">BOR Daily Run</h1>
          <p className="mt-1 text-sm text-gray-500">Manage and view BOR Daily Email Runs</p>
        ...
        <div className="card p-4 bg-yellow-50 border-yellow-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">The BOR daily scheduler is currently disabled. Click "Start Daily Run" to resume.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">BOR Daily Run History</h2>
        {dailyRuns.length === 0 ? (
          <div className="card p-12 text-center">
            <div className="text-gray-500 mb-4">No BOR daily runs found yet.</div>
            <p className="text-sm text-gray-400">BOR daily runs are automatically executed at {formatScheduledTime()}.</p>
          </div>
        ) : (
            ...
            {dailyRuns.map((run) => (
              <div key={run.id} className="card p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{run.total_batches > 0 ? Math.round((run.completed_batches / run.total_batches) * 100) : 0}%</p>
                      </div>
                    </div>
                    {run.error_message && (
                  ...
                  <Link to={`/jobs/${run.id}`} className="ml-4 px-4 py-2 bg-[#0B1020] text-white rounded-lg hover:bg-[#1a2235] transition-colors text-sm font-medium">
                    View Details →
                  </Link>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Schedule timing and email recipients here apply to BOR only.
                </p>
              </div>
            </div>
