...
export default function TEVDailyRun() {
  ...
  const loadNextRun = async () => {
    try {
      const data = await schedulerApi.getNextRun('tev')
      setNextRun(data)
    } catch (err: any) {
  ...
  const loadSchedulerSettings = async () => {
    try {
      const settings = await schedulerApi.getSettings('tev')
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setError('')
      await schedulerApi.updateSettings(settingsForm, 'tev')
  ...
  const handleToggleEnabled = async () => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ enabled: newEnabled }, 'tev')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
  ...
  const handleInputModeChange = async (mode: 'api' | 'uploaded') => {
    ...
    try {
      ...
      setError('')
      await schedulerApi.updateSettings({ input_mode: mode }, 'tev')
      await loadSchedulerSettings()
      await loadNextRun()
    } catch (err: any) {
  ...
  const loadDailyRuns = async () => {
    try {
      ...
      const dailyJobs = allJobs.filter((job: any) =>
        job.job_name &&
        (job.job_name.startsWith('Daily TEV Off Price Report -') ||
          job.job_name.startsWith('Daily TEV Metro Report -')),
      )
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">TEV Daily Run</h1>
          <p className="mt-1 text-sm text-gray-500">Manage and view TEV Daily Email Runs</p>
        ...
        <div className="card p-4 bg-yellow-50 border-yellow-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">The TEV daily scheduler is currently disabled. Click "Start Daily Run" to resume.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">TEV Daily Run History</h2>
        {dailyRuns.length === 0 ? (
          <div className="card p-12 text-center">
            <div className="text-gray-500 mb-4">No TEV daily runs found yet.</div>
            <p className="text-sm text-gray-400">TEV daily runs are automatically executed at {formatScheduledTime()}.</p>
          </div>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Schedule timing and email recipients here apply to TEV only.
                </p>
              </div>
            </div>
