...
import { catalogOldSkusApi } from '../../services/api'
import { useUser } from '../../contexts/UserContext'
import { canAccessOldSkus } from '../../lib/oldSkusAccess'
import type { CatalogOldSkusImportResult, CatalogOldSkusRecord } from '../../types'

const PAGE_SIZE = 50
const ACCEPTED = '.xlsx,.xlsm,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
const TEMPLATE_NAME = 'Old_Skus_Template.xlsx'
...
export default function OldSkusCatalogPage() {
  ...
  const [items, setItems] = useState<CatalogOldSkusRecord[]>([])
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await catalogOldSkusApi.list(PAGE_SIZE, page * PAGE_SIZE, search || undefined)
      ...
      setError(typeof detail === 'string' ? detail : 'Failed to load Old SKUs')
      setItems([])
      setTotal(0)
    } finally {
  ...
  useEffect(() => {
    if (userInfoLoading || !canUse) return
    void loadRecords()
  }, [userInfoLoading, canUse, loadRecords, refreshToken])

  const handleDownloadTemplate = async () => {
    setError(null)
    try {
      const blob = await catalogOldSkusApi.downloadTemplate()
  ...
  const handleImport = async (file: File) => {
    const confirmed = window.confirm(
      'Upload will replace the entire Old SKUs catalog with the rows in this file. Continue?',
    )
    if (!confirmed) {
    ...
    setMessage(null)
    try {
      const result: CatalogOldSkusImportResult = await catalogOldSkusApi.importFile(file)
      setMessage(
        `Imported ${result.imported.toLocaleString()} row(s)` +
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">You do not have access to the Old SKUs catalog.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Old SKUs</h1>
        <p className="mt-1 text-sm text-gray-600 dark:text-content-muted">
          Catalog of old SKUs with vendor name and UPC. Download the template, fill it using the same
          columns, then upload to replace the full list.
        </p>
      </div>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-border dark:bg-surface">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {importing ? 'Importing…' : 'Upload Old SKUs file'}
          </button>
        </div>
        <p className="mt-2 text-xs text-gray-500">
          Template columns: OLD SKU, Vendor Name, UPC Code. Import replaces all existing catalog
          rows.
        </p>
      </section>
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-border dark:bg-surface">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
          <div>
            <h2 className="text-sm font-semibold text-gray-800 dark:text-slate-100">Records</h2>
          ...
          <input
            ...
            placeholder="Search OLD SKU, vendor, UPC…"
                ...
                <tr>
                  <td colSpan={colCount} className="px-4 py-8 text-center text-gray-500">
                    {search
                      ...
                      : 'No records yet. Upload the Old SKUs spreadsheet to populate this list.'}
                  </td>
                </tr>
              ) : (
                items.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-surface-muted">
                      {columns.map((col) => (
                        <td
                          key={col}
                          className="px-3 py-2 whitespace-nowrap text-gray-800 dark:text-slate-200"
                        >
