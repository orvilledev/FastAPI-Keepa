...
export default function REFSchedulerCountdown() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      const [data, settings] = await Promise.all([
        Promise.race([schedulerApi.getNextRun('ref'), timeoutPromise]) as Promise<SchedulerStatus>,
        schedulerApi.getSettings('ref').catch(() => ({ enabled: true })),
      ])
  ...
  if (loading) {
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">Loading REF scheduler status...</div>
      </div>
    )
  }
    ...
    return (
      <div className="card p-6">
        <div className="text-center">
          <div className="text-red-600 font-medium mb-2">Error loading REF scheduler status</div>
    ...
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">
          {status?.message || 'REF Scheduler not configured'}
        </div>
      </div>
    )
  }
