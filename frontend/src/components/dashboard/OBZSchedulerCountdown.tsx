...
export default function OBZSchedulerCountdown() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      const [data, settings] = await Promise.all([
        Promise.race([schedulerApi.getNextRun('obz'), timeoutPromise]) as Promise<SchedulerStatus>,
        schedulerApi.getSettings('obz').catch(() => ({ enabled: true })),
      ])
  ...
  if (loading) {
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">Loading OBZ scheduler status...</div>
      </div>
    )
  }
    ...
    return (
      <div className="card p-6">
        <div className="text-center">
          <div className="text-red-600 font-medium mb-2">Error loading OBZ scheduler status</div>
    ...
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">
          {status?.message || 'OBZ Scheduler not configured'}
        </div>
      </div>
    )
  }
