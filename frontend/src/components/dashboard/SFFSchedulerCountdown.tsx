...
export default function SFFSchedulerCountdown() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      const [data, settings] = await Promise.all([
        Promise.race([schedulerApi.getNextRun('sff'), timeoutPromise]) as Promise<SchedulerStatus>,
        schedulerApi.getSettings('sff').catch(() => ({ enabled: true })),
      ])
  ...
  if (loading) {
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">Loading SFF scheduler status...</div>
      </div>
    )
  }
    ...
    return (
      <div className="card p-6">
        <div className="text-center">
          <div className="text-red-600 font-medium mb-2">Error loading SFF scheduler status</div>
      ...
      <div className="bg-gray-100 rounded-xl shadow p-6 border border-gray-300">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <h3 className="text-lg font-semibold mb-2 text-gray-900">SFF Keepa Off Price Daily Run</h3>
    ...
    return (
      <div className="card p-6">
        <div className="text-center text-gray-500">
          {status?.message || 'SFF Scheduler not configured'}
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <h3 className="text-lg font-semibold mb-2 text-white">SFF Keepa Off Price Daily Run</h3>
