...
export default function VendorRunCard({
  ...
  const dailyRunPath = VENDOR_PATHS.has(category) ? `/daily-run/${category}` : null
  ...
  if (!vendor.enabled) {
    ...
    if (dailyRunPath) {
      return (
        <Link
          to={dailyRunPath}
          className="block bg-gray-100 rounded-xl shadow p-6 border border-gray-300 transition-colors hover:border-gray-400 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#81B81D]/60"
          aria-label={`Open ${code} Daily Run`}
        >
          {stoppedBody}
        </Link>
      )
    ...
    return (
      <div className="bg-gray-100 rounded-xl shadow p-6 border border-gray-300">{stoppedBody}</div>
    )
  }
  ...
  if (!vendor.next_run_time) {
    if (dailyRunPath) {
      return (
        <Link
          to={dailyRunPath}
          className="card block p-6 transition-colors hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#81B81D]/60"
          aria-label={`Open ${code} Daily Run`}
        >
          <div className="text-center text-gray-500">{code} Scheduler not configured — open to set up</div>
        </Link>
      )
    }
    return (
  ...
  const cardInner = (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="shrink-0 sm:text-right">
        ...
        {onReminderToggle && (
          <div
            className="mt-3 flex items-center gap-2 sm:justify-end"
        ...
        {dailyRunPath && (
          <p className="mt-3 text-[11px] text-white/50 sm:text-right">Open Daily Run →</p>
        )}
      </div>
    </div>
  )
  ...
  if (!dailyRunPath) {
    return (
      <div className="rounded-xl shadow-xl p-4 sm:p-5 text-white border bg-[#404040] border-white/20">
        {cardInner}
      </div>
    )
  }
  ...
  return (
    <Link
      to={dailyRunPath}
      className="block rounded-xl shadow-xl p-4 sm:p-5 text-white border bg-[#404040] border-white/20 transition hover:border-white/40 hover:bg-[#4a4a4a] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#81B81D]/70"
      aria-label={`Open ${code} Daily Run`}
    >
      {cardInner}
    </Link>
