...
def _category_daily_run_lock(category: str) -> asyncio.Lock:
    normalized = _normalize_vendor(category)
    return _category_daily_run_locks.setdefault(normalized, asyncio.Lock())
...
async def try_acquire_category_daily_run_lock(category: str) -> bool:
    ...
    lock = _category_daily_run_lock(category)
    if lock.locked():
        return False
    await lock.acquire()
    return True
...
async def acquire_category_daily_run_lock(category: str) -> None:
    """Wait until this vendor is free (Trigger Import / forced rerun)."""
    lock = _category_daily_run_lock(category)
    if lock.locked():
        logger.info(
            "%s daily run already in progress; waiting to start queued import/trigger",
            _normalize_vendor(category).upper(),
        )
    await lock.acquire()
...
def claim_completion_email_send(db: Client, job_id: str) -> bool:
    """Atomically reserve a job so only one worker sends its completion email."""
...
def release_completion_email_claim(db: Client, job_id: str) -> None:
    """Clear the send reservation so a later retry can email this job."""
    try:
        db.table("batch_jobs").update({"completion_email_sent_at": None}).eq("id", job_id).execute()
    except Exception as exc:
        logger.warning("Could not release completion email claim for job %s: %s", job_id, exc)
...
def send_daily_run_completion_email_for_job(
    ...
    *,
    ...
    calendar day. Successful sends keep ``completion_email_sent_at``. Failed or
    skipped sends release that reservation so the job can be retried.
    """
    ...
    try:
        ...
        if not job_resp.data:
            ...
            release_completion_email_claim(db, job_id_str)
            return False
        ...
        if not sent:
            logger.warning("Retrying completion email for job %s", job_id_str)
            sent = mailer.send_csv_report(**send_kwargs)
        if sent:
        ...
        logger.exception("Completion email step failed for job %s", job_id_str)
        release_completion_email_claim(db, job_id_str)
        if is_daily_run:
            refresh_live_analytics_snapshots(db)
        return False
